...
def _load_raw_trajectory(trial_dir: Path) -> "torch.Tensor | None":
    """(2, T) raw (pre-normalization) offline-retracked (x, y) position —
    the "before concatenation" counterpart to
    dataset_ctc.load_trajectory_variable, same reasoning as
    _load_raw_imu above: several of these get concatenated first, then
    normalized ONCE over the full synthetic sequence, not per-segment.
    Returns None if the file is missing or has fewer than 2 detected
    frames — the caller drops this pick from the whole synthetic
    sample's trajectory contribution when that happens (see
    ConcatLetterDatasetCTC.__getitem__), same "auxiliary target can
    legitimately be absent" rule dataset_ctc.load_trajectory_variable
    documents."""
    ...
    return torch.from_numpy(traj.astype(np.float32))   # (2, T) — NOT normalized here, see docstring
...
class ConcatLetterDatasetCTC(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        picks = []
        imu_segs = []
        imu_target_segs = []   # only populated for motion_target=='fingertip_imu' (trajectory
                                 # is handled entirely separately below, since it has to survive
                                 # EVEN when a pick's own IMU segment loaded fine)
        ...
        if self.motion_target == 'fingertip_imu':
            ...
            has_motion_target = True   # load_imu_variable/_load_raw_imu never return None for
                                         # a pick that already survived the imu_segs check above
        else:   # 'trajectory'
            # Loaded here, AFTER `picks` is finalized — trajectory
            # availability is checked SEPARATELY from IMU availability
            # (unlike the fingertip_imu branch above, where a missing
            # target_seg disqualifies a pick from `picks` entirely): a
            # pick can have perfectly good watch/fingertip IMU but no
            # trajectory re-track yet (see extract_fingertip_trajectory.py
            # — an offline, run-after-the-fact process that may not cover
            # every trial), so gating `picks` itself on trajectory
            # availability would needlessly throw away good IMU/audio
            # data. Instead: if EVERY pick has trajectory data, use it;
            # if ANY pick is missing it, the whole synthetic sample's
            # trajectory contribution is dropped (has_motion_target=False)
            # rather than splicing a gap where a real segment should be —
            # an occasional skipped auxiliary loss is harmless (same rule
            # dataset_ctc.load_trajectory_variable follows), but a
            # spliced-together fake trajectory would actively mislead it.
            traj_segs = [_load_raw_trajectory(trial_dir) for trial_dir, _ in picks]
            if all(t is not None for t in traj_segs):
                traj_pieces = []
                for t in traj_segs:
                    traj_pieces.append(t)
                    traj_pieces.append(t[:, -1:].repeat(1, gap_steps))
                full_imu_target = torch.cat(traj_pieces[:-1], dim=1)
                full_imu_target = torch.from_numpy(_robust_origin_normalize(full_imu_target.numpy()))
                full_imu_target = full_imu_target / config_ctc.TRAJECTORY_PIXEL_SCALE   # see
                                                                                            # dataset_ctc.
                                                                                            # load_trajectory_
                                                                                            # variable's
                                                                                            # identical step
                ...
                full_imu_target = torch.zeros(2, 2, dtype=torch.float32)
                has_motion_target = False

        target = torch.tensor([label + 1 for _, label in picks], dtype=torch.long)
        return mel, full_imu, target, full_imu_target, has_motion_target, surface_mel