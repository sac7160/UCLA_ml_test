...
import config_ctc
from dataset_ctc import load_audio_variable, load_imu_variable, load_trajectory_variable
...
class RealTextDatasetCTC(Dataset):
    ...
    — text is the trial's ground-truth string (a word, or a sentence).
    `classes` must be the SAME letter list the model's classifier head
    was sized against (e.g. a-z) — any character in `text` that's
    neither a letter in `classes` NOR a space (punctuation, digits) is
    simply skipped when building the target, not an error.

    Spaces ARE kept in the target (mapped to a dedicated SPACE class,
    index len(classes)+1 — see model_ctc.py's classifier/
    ctc_greedy_decode) — an earlier version of this dropped every space
    entirely, which meant the model had no class it could even output a
    space as, and sentence predictions came back with real word
    boundaries silently collapsed into one run-on string (see the chat
    this was fixed in).
    ...
    def __init__(self, samples: list, classes: list,
                 ...
                 use_space: bool = True, denoise_fingertip: bool = True,
        ...
        # Only meaningful (and only ever assigned into a target) when
        # use_space — see __getitem__. Must match how the model this
        # feeds was built (model_ctc.LetterCTCNet's own use_space) or the
        # target indices won't line up with what the classifier head can
        # actually output.
        self.space_class_idx = len(classes) + 1
        self.finger = finger
    ...
    def __getitem__(self, idx):
        ...
        if self.motion_target == 'fingertip_imu':
            imu_target = load_imu_variable(trial_dir, 'fingertip', self.finger, t_start, t_end,
                                            robust_origin=True, denoise=self.denoise_fingertip)
            has_motion_target = True
        else:   # 'trajectory'
            traj = load_trajectory_variable(trial_dir, t_start, t_end)
        ...
        for c in text:
            if c == ' ':
                # use_space=False: spaces are dropped entirely (not even a
                # gap marker) — a sentence's target becomes a continuous
                # run of letters, same as before space support existed.
                # This is the deliberate ALTERNATIVE this flag exists to
                # test — see model_ctc.LetterCTCNet's use_space docstring
                # for the reasoning (space is a fundamentally different
                # KIND of signal than a letter, and forcing one classifier
                # to discriminate both may hurt letter recognition itself).
                if self.use_space:
                    target_indices.append(self.space_class_idx)
            elif c in self.char_to_idx:
                target_indices.append(self.char_to_idx[c] + 1)
            # anything else (punctuation, digits) is silently skipped, same as before
        target = torch.tensor(target_indices, dtype=torch.long)
        if len(target) == 0:
            # every character in `text` fell outside `classes`/space (shouldn't
            # normally happen for real English text against a-z, but
