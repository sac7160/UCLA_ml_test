...
import config_ctc
from dataset_ctc import _MEL, _DB, _resample_to_rate, _smooth
...
def _load_raw_imu(trial_dir: Path, imu_source: str, finger: str) -> "torch.Tensor | None":
    ...
    imu = _smooth(imu.astype(np.float32), smooth_window)
    return torch.from_numpy(imu.astype(np.float32))


def _load_raw_trajectory(trial_dir: Path, finger: str) -> "torch.Tensor | None":
    """(2, T) raw (pre-normalization) fingertip (x, y) position — the
    "before concatenation" counterpart to dataset_ctc.load_trajectory_variable,
    same reasoning as _load_raw_imu above: several of these get
    concatenated first, then normalized ONCE over the full synthetic
    sequence, not per-segment. Returns None if unavailable (poor
    fingertip tracking on that source trial) — the caller drops that
    pick from the WHOLE synthetic sample if this happens (see
    ConcatLetterDatasetCTC.__getitem__), the same "audio/imu/traj/target
    must all agree on the same letter sequence" rule the IMU check
    already enforces there."""
    df = pd.read_csv(trial_dir / config_ctc.IMU_FILENAMES['fingertip'])
    df = df[(df['finger'] == finger) & (df['detected'])]
    if len(df) < 2:
        return None
    ...
    smooth_window = max(1, int(config_ctc.IMU_SMOOTH_WINDOW_SEC * config_ctc.IMU_RESAMPLE_HZ))
    traj = _smooth(traj.astype(np.float32), smooth_window)   # per-segment, same reasoning as above
    return torch.from_numpy(traj.astype(np.float32))
...
class ConcatLetterDatasetCTC(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # audio or IMU — audio/imu/traj/target all have to agree on the
        # exact same letter sequence, so a pick that fails to load for
        # IMU can't just be silently dropped from IMU alone while audio
        # still includes it (that used to be a real bug here: audio built
        # from ALL n picks, IMU/target only from the ones that survived
        # IMU loading, so a fusion-modality sample could end up with a
        # 4-letter audio span paired against a 3-letter target). IMU is
        # loaded here once and reused below rather than reloaded, since
        # that load is exactly the check being performed. Trajectory is
        # allowed to be missing for an otherwise-usable pick (see
        # _load_raw_trajectory's docstring) — that pick still counts for
        # audio/imu/target, it just contributes no trajectory segment,
        # same "auxiliary target can legitimately be absent" rule
        # load_trajectory_variable documents.
        picks = []
        imu_segs = []
        traj_segs = []   # None entries allowed — see above
        for trial_dir, label in candidates:
            seg = _load_raw_imu(trial_dir, self.imu_source, self.finger)
            if seg is None:
                continue
            picks.append((trial_dir, label))
            imu_segs.append(seg)
            traj_segs.append(_load_raw_trajectory(trial_dir, self.finger))
        if not picks:
        ...
        if all(t is not None for t in traj_segs):
            traj_pieces = []
            for t in traj_segs:
                traj_pieces.append(t)
                traj_pieces.append(t[:, -1:].repeat(1, gap_steps))
            full_traj = torch.cat(traj_pieces[:-1], dim=1)
            traj_mean = full_traj.mean(dim=1, keepdim=True)
            traj_std = full_traj.std(dim=1, keepdim=True) + 1e-6
            full_traj = (full_traj - traj_mean) / traj_std
        else:
            # Any one pick missing trajectory data disqualifies the whole
            # synthetic sample from this auxiliary task, rather than
            # trying to splice a gap where a real segment should be — an
            # occasional skipped auxiliary loss is harmless (see
            # load_trajectory_variable's docstring); a spliced-together
            # fake trajectory would actively mislead the auxiliary task.
            full_traj = None
