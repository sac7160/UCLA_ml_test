...
class ConcatLetterDatasetCTC(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for trial_dir, label in candidates:
            ...
            if seg is None:
                continue
            target_seg = _load_raw_imu(trial_dir, 'fingertip', self.finger)
        ...
        target_mean = full_imu_target.mean(dim=1, keepdim=True)
        target_std = full_imu_target.std(dim=1, keepdim=True) + 1e-6
        full_imu_target = (full_imu_target - target_mean) / target_std
