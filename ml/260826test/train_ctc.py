...
def compute_motion_loss(imu_recon: torch.Tensor, imu_target: torch.Tensor, imu_target_lengths: torch.Tensor,
    ...
    """MSE between the IMU decoder's reconstruction and the real
    FINGERTIP IMU signal (6 channels: accel/gyro) — the reconstruction
    target is always the fingertip IMU source, regardless of what
    --imu-source the model's actual INPUT uses (see
    dataset_ctc.LetterDatasetCTC's imu_target, loaded independently of
    the run's own --imu-source) — mirroring exactly how the Mic
    decoder's target is always the surface mic recording regardless of
    --audio-source (see compute_spec_loss). When --imu-source is already
    'fingertip' this degenerates to plain self-reconstruction (input and
    target are the same recording) — a harmless, expected special case,
    not a bug.
    ...
    imu_target lives at the fingertip source's own (usually close but
    not always identical) time resolution (imu_target_lengths) —
    ...
    for i in range(imu_recon.shape[0]):
        ...
        if recon_len < 1 or target_len < 2:
            continue
        recon_i = imu_recon[i, :recon_len, :]                            # (recon_len, 6)
        target_i = imu_target[i, :, :target_len].unsqueeze(0)              # (1, 6, target_len)
        target_resized = F.interpolate(target_i, size=recon_len, mode='linear', align_corners=False)
        target_resized = target_resized.squeeze(0).transpose(0, 1)          # (recon_len, 6)
...
def main():
    ...
    parser.add_argument('--sequence-encoder', choices=['gru', 'transformer'], default='gru',
                         help='which architecture reads the fused/gated audio+IMU features over time and '
                              ...
                              '(default) is the existing 2-layer bidirectional GRU; "transformer" swaps in '
                              'a Transformer encoder (self-attention) instead, letting every timestep '
                              'attend directly to every other timestep rather than only through a '
                              'recurrent hidden state. Everything else in the model (audio/IMU encoders, '
                              'gated fusion, IMU/Mic decoders, classifier head, CTC loss) is byte-for-byte '
                              'identical either way — this flag isolates the comparison to exactly this one '
                              'component. See --transformer-layers/--transformer-nhead for the '
                              'Transformer\'s own size knobs (ignored when --sequence-encoder gru).')
    ...
    parser.add_argument('--motion-loss-weight', type=float, default=0.0,
                         help='weight for the IMU decoder\'s auxiliary reconstruction loss — reads z_imu '
                              '(the TCN IMU encoder\'s own latent, BEFORE fusion) and is trained to '
                              'reconstruct the real FINGERTIP IMU signal (accel/gyro, 6 channels), ALWAYS '
                              'from the \'fingertip\' source regardless of what --imu-source the model\'s '
                              'actual input uses (mirroring how the Mic decoder always targets the surface '
                              'mic recording regardless of --audio-source): total_loss = ctc_loss + '
                              'motion_loss_weight * mse(reconstructed_imu, real_fingertip_imu) [+ '
                              'spec_loss_weight * ... , see --spec-loss-weight]. With --imu-source watch, '
                              'this pushes z_imu toward "what would the camera-tracked fingertip sensor '
                              'have read", not mere self-reconstruction. 0.0 (default) disables this loss '
                              'term entirely — z_imu is still trained purely by CTC loss backpropagating '
                              'through it either way; this only controls whether it\'s ALSO nudged toward '
                              'matching the real fingertip signal. Try 0.1-0.5 as a starting point if '
                              'enabling it.')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for (audio, audio_len, imu, imu_len, targets, target_len, imu_target, imu_target_len,
            ...
            optimizer.zero_grad()
            log_probs, out_len, imu_recon, spec_recon = model(audio, audio_len, imu, imu_len)
            loss = loss_fn(log_probs, targets, out_len, target_len)
