...
def compute_motion_loss(traj_pred: torch.Tensor, traj_padded: torch.Tensor, traj_lengths: torch.Tensor,
                         has_traj: torch.Tensor, out_lengths: torch.Tensor):
    """MSE between traj_decoder's reconstruction of the latent motion
    bottleneck (traj_pred, at the RNN's own input time resolution) and
    the real normalized trajectory (traj_padded, at
    config_ctc.IMU_RESAMPLE_HZ — a DIFFERENT resolution, hence the per-
    sample interpolation below), averaged only over samples that
    actually have real trajectory ground truth (has_traj — see
    dataset_ctc.load_trajectory_variable()'s docstring for why some
    samples legitimately don't). Returns None (not zero) if no sample in
    this batch has trajectory ground truth, so the caller can skip adding
    it to the loss entirely rather than adding a real zero — a
    distinction that matters for anything logging/averaging this loss
    over epochs."""
    if not has_traj.any():
        return None
    losses = []
...
def evaluate(model, loader, classes, device, n_samples_to_show: int = 8):
    ...
    with torch.no_grad():
        for audio, audio_len, imu, imu_len, targets, target_len, _traj, _traj_len, _has_traj in loader:
            ...
            log_probs, out_len, _traj_pred = model(audio, audio_len, imu, imu_len)
...
def evaluate_word_level(model, text_val_samples: list, classes: list, device, modality: str,
    ...
    return normalized_edit_distance, mean_edit_distance, accuracy, examples
...
def main():
    ...
    parser.add_argument('--motion-dim', type=int, default=16,
                         help='width of the "latent fingertip motion" bottleneck between the audio/IMU '
                              'encoders and the RNN (see model_ctc.py\'s motion_head) — everything the RNN '
                              'sees has to pass through this many channels, not the full fused feature '
                              'width, which is what makes it an actual architectural bottleneck rather than '
                              'a side-output. Deliberately NOT 2 (a literal (x, y) pair) — that would be too '
                              'severe a compression for letter decoding to survive; this default (16) leaves '
                              'the network some room while still being a real constraint.')
    parser.add_argument('--motion-loss-weight', type=float, default=0.0,
                         help='weight for the auxiliary motion-reconstruction loss — a small head '
                              '(traj_decoder) reads the latent motion bottleneck and is trained to '
                              'reconstruct the real, NORMALIZED (no absolute coordinates — see '
                              ...
                              'real_trajectory). Inspired by Watch Your Mouth\'s use of spatiotemporal mouth '
                              'geometry as a rich intermediate representation the model must route '
                              'information through before decoding to text. 0.0 (default) disables the loss '
                              'term entirely (the bottleneck itself still exists and is still trained purely '
                              'by CTC loss backpropagating through it — --motion-loss-weight only controls '
                              'whether it\'s ALSO nudged toward matching real physical motion). Try 0.1-0.5 '
                              'as a starting point if enabling it.')
    ...
    parser.add_argument('--use-sentence-trials', action='store_true',
                         help='same as --use-word-trials but for sentence/ trials — spaces are dropped from '
                              'the target (the model spells the sentence\'s letters in order, not word '
                              'boundaries). Sentences are the longest, most error-prone-to-align real trials '
                              'available; consider starting with --use-word-trials alone before adding this.')
    ...
    model = build_model_ctc(args.modality, n_classes=len(args.classes), dropout=args.dropout,
                             motion_dim=args.motion_dim).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        model.train()
        running_loss, running_motion_loss, n_batches, n_motion_batches = 0.0, 0.0, 0, 0
        ...
        for audio, audio_len, imu, imu_len, targets, target_len, traj, traj_len, has_traj in pbar:
            ...
            optimizer.zero_grad()
            log_probs, out_len, traj_pred = model(audio, audio_len, imu, imu_len)
            loss = loss_fn(log_probs, targets, out_len, target_len)
            if args.motion_loss_weight > 0:
                motion_loss = compute_motion_loss(traj_pred, traj, traj_len, has_traj, out_len)
        ...
        if has_word_val and epoch % args.word_eval_every == 0:
            norm_ed, mean_ed, word_acc, word_examples = evaluate_word_level(
