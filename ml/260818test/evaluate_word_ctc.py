...
def main():
    ...
    motion_dim = ckpt.get('motion_dim', 16)   # older checkpoints predating the motion bottleneck
                                                # don't have this key — 16 matches train_ctc.py's own
                                                # default, so this only matters for checkpoints trained
                                                # with a non-default --motion-dim
    ...
    model = build_model_ctc(modality, n_classes=len(classes), motion_dim=motion_dim).to(device)
