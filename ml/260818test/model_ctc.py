...
class LetterCTCNet(nn.Module):
    ...
    def __init__(self, modality: str, n_classes: int, imu_in_channels: int,
                 ...
                 dropout: float = 0.3, motion_dim: int = 16):
        super().__init__()
        ...
        if modality == 'fusion':
            ...
            pre_motion_dim = fusion_dim
        elif modality == 'audio':
            pre_motion_dim = audio_embed
        else:
            pre_motion_dim = imu_embed

        # "Latent fingertip motion" bottleneck — IMU+Audio -> [latent
        # fingertip motion] -> characters, instead of the fused features
        # feeding the RNN directly. Inspired by Watch Your Mouth's use of
        # a rich intermediate representation of the physical phenomenon
        # (there: spatiotemporal mouth geometry; here: spatiotemporal
        # fingertip motion) as an encoding stage BEFORE decoding to text
        # — everything the RNN ever sees has to pass through this
        # motion_dim-wide channel, not the full pre_motion_dim features,
        # which is what makes this an actual architectural bottleneck
        # rather than a side-output bolted onto the existing features (an
        # earlier version of this did exactly that — an auxiliary
        # trajectory-prediction head reading the RNN's OUTPUT — and was
        # removed for being a side task rather than a real information
        # channel; see the chat this replaced it in).
        #
        # motion_dim is deliberately NOT 2 (a literal (x, y) pair) — that
        # would be too severe a compression for the letter-decoding task
        # itself to survive, throwing away nearly everything except raw
        # position. Instead, motion_head projects down to a moderate-
        # width motion_dim (default 16), and a separate, much smaller
        # traj_decoder (motion_dim -> 2) reads only THAT to reconstruct
        # (x, y) for the auxiliary loss (see train_ctc.py's
        # --motion-loss-weight) — the bottleneck is regularized to behave
        # like real fingertip motion without being forced to discard
        # everything else useful for classification. Tanh bounds the
        # latent to a fixed range, similar in spirit to how a real
        # geometric coordinate is bounded, rather than letting it grow
        # arbitrarily large the way an unconstrained hidden state can.
        self.motion_head = nn.Sequential(nn.Linear(pre_motion_dim, motion_dim), nn.Tanh())
        self.traj_decoder = nn.Linear(motion_dim, 2)   # motion_dim -> normalized (x, y), auxiliary-loss only

        self.rnn = nn.GRU(motion_dim, rnn_hidden, num_layers=2, batch_first=True,
        ...
        self.last_motion = None  # (B, T', motion_dim) from the most recent forward() call — the
                                  # actual "latent fingertip motion" the RNN read; kept around for the
                                  # same purely-diagnostic reason as last_gate
    ...
    def forward(self, audio: "torch.Tensor | None", audio_lengths: "torch.Tensor | None",
        ...
        """Returns (log_probs, output_lengths, traj_pred): log_probs is
        (T', B, n_classes+1) — CTC's expected (time-major) layout —
        output_lengths is (B,), each sample's real (unpadded) timestep
        count after the encoder's downsampling, and traj_pred is (B, T',
        2), the traj_decoder's reconstruction of (x, y) from the latent
        motion bottleneck (see self.motion_head/self.traj_decoder above)
        — callers that don't use the motion auxiliary task
        (--motion-loss-weight=0) can simply ignore this third return
        value; it's always computed (cheap — one small Linear layer) but
        costs nothing unless something actually trains against it."""
        ...
        motion = self.motion_head(feat)             # (B, T', motion_dim) — the actual bottleneck;
                                                       # everything downstream sees ONLY this
        self.last_motion = motion.detach()
        traj_pred = self.traj_decoder(motion)         # (B, T', 2) — auxiliary reconstruction, doesn't
                                                         # feed back into the RNN below

        rnn_out, _ = self.rnn(motion)               # (B, T', 2*rnn_hidden) — reads the bottleneck,
                                                       # never the raw pre-motion features
        rnn_out = self.dropout(rnn_out)
        logits = self.classifier(rnn_out)          # (B, T', n_classes+1)
        log_probs = F.log_softmax(logits, dim=2)
        return log_probs.transpose(0, 1), out_lengths, traj_pred   # (T', B, n_classes+1), (B,), (B, T', 2)


def build_model_ctc(modality: str, n_classes: int,
                     imu_in_channels: int = len(config_ctc.IMU_CHANNELS),
                     dropout: float = 0.3, motion_dim: int = 16) -> LetterCTCNet:
    return LetterCTCNet(modality, n_classes, imu_in_channels, dropout=dropout, motion_dim=motion_dim)
