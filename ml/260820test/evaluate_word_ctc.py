...
def main():
    ...
    model = build_model_ctc(modality, n_classes=len(classes), rnn_hidden=rnn_hidden).to(device)
    ...
    with torch.no_grad():
        for participant_name, pdir in participant_dirs:
            ...
            for trial_dir in trial_dirs:
                ...
                if args.decode == 'beam':
                    raw_pred = ctc_beam_search_decode(log_probs, out_len, classes, beam_width=args.beam_width)[0]
                else:
