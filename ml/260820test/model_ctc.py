...
def ctc_beam_search_decode(log_probs: torch.Tensor, lengths: torch.Tensor, classes: list,
    ...
    log_probs: (T, B, n_classes+2) as returned by LetterCTCNet.forward()
    — same +2 space-class convention as ctc_greedy_decode (index 0 =
    blank, index len(classes)+1 = space). Returns a list of B decoded
    strings — the single best (highest-probability) hypothesis per
    sample. beam_width trades decode time for search thoroughness —
    larger finds better hypotheses but takes proportionally longer;
    beam_width=1 degenerates to (a slightly slower, but equivalent)
    greedy decode."""
    space_idx = len(classes) + 1
    B = log_probs.shape[1]
