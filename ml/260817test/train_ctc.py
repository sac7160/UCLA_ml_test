...
def evaluate(model, loader, classes, device, n_samples_to_show: int = 8):
    ...
    with torch.no_grad():
        for audio, audio_len, imu, imu_len, targets, target_len in loader:
...
def main():
    ...
    model = build_model_ctc(args.modality, n_classes=len(args.classes), dropout=args.dropout).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for audio, audio_len, imu, imu_len, targets, target_len in pbar:
            audio, imu = audio.to(device), imu.to(device)
            audio_len, imu_len = audio_len.to(device), imu_len.to(device)
            targets, target_len = targets.to(device), target_len.to(device)
