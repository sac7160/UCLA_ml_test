...
class CNNSequenceEncoder(nn.Module):
    """A simple stacked 1D-convolutional sequence encoder — same role as
    the BiGRU/BiLSTM/Transformer paths in LetterCTCNet (reads (B, T,
    in_channels) -> (B, T, out_channels), T never downsampled — CTC
    needs a per-timestep output aligned with the original time axis),
    but using plain feedforward dilated convolutions instead of any
    recurrent or self-attention mechanism.

    Reuses the SAME dilated-residual block design as IMUEncoderTCN's own
    _TCNBlock (dilations 1, 2, 4, ... — growing receptive field without
    ever shrinking T). Meant as the LIGHTEST-WEIGHT, most stable-to-train
    of the --sequence-encoder choices: no recurrent backprop-through-time
    memory overhead (unlike GRU/LSTM, which must keep every intermediate
    hidden state alive across the whole sequence for backward()) and no
    O(T^2) self-attention memory (unlike Transformer) — useful
    specifically when training keeps crashing on hardware that can't
    reliably sustain those two, rather than genuine model-capacity
    reasons (see the chat this was added in)."""

    def __init__(self, in_channels: int, out_channels: int, n_blocks: int = 3,
                 kernel_size: int = 3, dropout: float = 0.3):
        super().__init__()
        self.in_proj = nn.Conv1d(in_channels, out_channels, kernel_size=1)
        dilations = tuple(2 ** i for i in range(n_blocks))   # 1, 2, 4, ... — same convention as
                                                                # IMUEncoderTCN's own default
        self.blocks = nn.ModuleList([
            _TCNBlock(out_channels, kernel_size, d, dropout) for d in dilations
        ])

    def forward(self, x: torch.Tensor) -> torch.Tensor:   # (B, T, in_channels) -> (B, T, out_channels)
        x = x.transpose(1, 2)          # (B, in_channels, T) — Conv1d expects channels-first
        x = self.in_proj(x)
        for block in self.blocks:
            x = block(x)                # (B, out_channels, T), T unchanged throughout
...
class LetterCTCNet(nn.Module):
    ...
    def __init__(self, modality: str, n_classes: int, imu_in_channels: int,
        ...
        if sequence_encoder not in ('gru', 'lstm', 'transformer', 'cnn'):
            raise ValueError(f'sequence_encoder must be gru/lstm/transformer/cnn, got "{sequence_encoder}"')
        ...
        if sequence_encoder == 'gru':
            ...
            # See CNNSequenceEncoder's own docstring — the lightest-weight,
            # most training-stable option of the four (no recurrent
            # backprop-through-time memory, no O(T^2) attention memory),
            # useful specifically when hardware stability is the limiting
            # factor rather than model capacity (see the chat this was
            # added in). Output width rnn_hidden*2 — matching GRU/LSTM/
            # Transformer exactly, so the classifier head below is
            # IDENTICAL regardless of which sequence encoder was used.
            self.cnn_encoder = CNNSequenceEncoder(rnn_in, rnn_hidden * 2, dropout=dropout)
    ...
    def forward(self, audio: "torch.Tensor | None", audio_lengths: "torch.Tensor | None",
        ...
        if self.sequence_encoder_type in ('gru', 'lstm'):
        ...
        elif self.sequence_encoder_type == 'transformer':
            ...
            seq_out = self.cnn_encoder(feat)   # (B, T', 2*rnn_hidden) — see CNNSequenceEncoder's
                                                  # own docstring; no padding mask needed the way
                                                  # Transformer's self-attention does, since a plain
                                                  # convolution only ever looks at a FIXED-size local
                                                  # window around each timestep (never the whole
                                                  # sequence at once), so padded positions can't
                                                  # contaminate real ones beyond that window — CTC's
                                                  # own out_lengths (already computed above) is
                                                  # enough to keep the loss/decoder from reading
                                                  # padding as real signal, same as the GRU/LSTM path.
