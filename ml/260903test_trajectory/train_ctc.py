...
def main():
    ...
    parser.add_argument('--sequence-encoder', choices=['gru', 'lstm', 'transformer', 'cnn'], default='gru',
                         help='which architecture reads the fused/gated audio+IMU features over time and '
                              ...
                              'only through a recurrent hidden state; "cnn" swaps in a lightweight stacked-'
                              'dilated-convolution encoder instead (see model_ctc.CNNSequenceEncoder) — no '
                              'recurrent backprop-through-time memory (unlike gru/lstm) and no O(T^2) '
                              'attention memory (unlike transformer), the most training-stable/lowest-memory '
                              'option of the four when hardware stability (not model capacity) is the '
                              'limiting factor. Everything else in the model (audio/IMU encoders, gated '
                              'fusion, IMU/Mic decoders, classifier head, CTC loss) is byte-for-byte '
                              ...
                              'Transformer\'s own size knobs (ignored for gru/lstm/cnn).')
