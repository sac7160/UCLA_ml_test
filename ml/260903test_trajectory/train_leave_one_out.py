...
def main():
    ...
    parser.add_argument('--include-letters', action='store_true',
                         help='also evaluate held-out letter trials per fold — see '
                              'evaluate_word_ctc.py\'s own --include-letters. Note: unlike word/sentence '
                              '(which are genuinely 100%% held-out since the held-out participant '
                              'contributed nothing to training), letter trials from OTHER participants '
                              'were used for training, so only the held-out participant\'s OWN letters '
                              'are evaluated here (via --participants [held_out] below) — still a fair, '
                              'genuinely unseen-user number, just scoped to letters specifically.')
    ...
    for held_out in held_out_list:
        ...
        eval_args = argparse.Namespace(
            ...
            include_sentences=args.include_sentences, include_letters=args.include_letters,
            splits_json=None,
        ...
        row.update(_summarize(df, 'letter', has_dict))
